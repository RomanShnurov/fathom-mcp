...
def test_local_build():
    ...
    result = subprocess.run([sys.executable, "-c", "import contextfs; print('✅ Module import successful')"], 
    ...
    # Test CLI
    print("\n🔄 Testing CLI...")
    result = subprocess.run([sys.executable, "-m", "contextfs", "--help"], 
                          capture_output=True, text=True)
    if result.returncode == 0:
...
def test_docker_quick():
    ...
    improvements = [
        ...
        ("multi-stage", "✅ Uses multi-stage build" if "FROM python:3.12-slim as builder" in dockerfile_content.lower() else "❌ Missing multi-stage build")
    ]
